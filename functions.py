...
def total_meal_price(meal_price, tip = 0.15):
    ...
    meal_price = meal_price + (meal_price * tip)
    return meal_price
...
def sign_and_parity(num4):
    
    out_list = []
    if (num4 > 0):
        out_list.append("Positive")
    else:
...
def full_title(fname, job = "Engineer"):
...
def write_letter(rname, job, sname):

    print(f"""Dear {job} {rname}, I think you are amazing! Sincerely, {sname}""")
###############################################################################
